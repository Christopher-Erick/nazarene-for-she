...
import { supportCta, type HeaderNavItem, type NavItem } from "@/lib/data/navigation";
...
export function SiteHeader({
  ...
  return (
    <>
      ...
      >
        <div className="mx-auto flex h-full max-w-7xl items-center justify-between gap-3 px-4 py-2 sm:px-8">
          <Link href="/" className="brand-lockup shrink-0" aria-label={`${site.name} home`}>
            ...
            <span className="min-w-0 leading-none">
              ...
              <span className="brand-tagline mt-1 hidden text-[0.7rem] italic xl:block">
                {site.shortTagline}
              </span>
            </span>
          </Link>

          <PrimaryNav items={headerItems} pathname={pathname} />

          <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
            <ButtonLink href={supportCta.href} className="header-cta">
              <span className="lg:hidden">Support</span>
              <span className="hidden lg:inline">{supportCta.label}</span>
            </ButtonLink>
      ...
      <MobileMenu open={open} pathname={pathname} navItems={menuItems} onClose={() => setOpen(false)} />
    </>
  );
}
